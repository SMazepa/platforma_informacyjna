 ...
 function statArticle(){
   ...
   if ($row) {
     foreach ($row as $key => $val) {
       ...
       $sql = mysqli_query($this->$link,
       "SELECT * FROM `artykul`
        LEFT JOIN `statystyka` ON `statystyka`.`art_id` = `artykul`.`art_id`
        WHERE `artykul`.`art_id` = $art_id
        GROUP BY `statystyka`.`stat_data`
        ORDER BY `artykul`.`art_id` DESC;");
        $days = mysqli_fetch_all($sql,MYSQLI_ASSOC);

        if ($days) {
          foreach ($days as $k => $v) {
            $vdata = $v['stat_data'];
            $sql = mysqli_query($this->$link,
            "SELECT COUNT(*) FROM `statystyka` WHERE `stat_data` = '$vdata'; ");
            $count = mysqli_fetch_all($sql,MYSQLI_ASSOC);

            $all_days .= '<p>Dnia: '.$v['stat_data'].' wyświetleń: '.$count[0]['COUNT(*)'].'</p>';
          }
        }
     ...
     $content .= '
     <div class="noData">Brak statystyk w bazie</div>
     ';
   }
   echo $content;
 }
...
// dodanie nowego artykulu
 function addArticle(){
   if (!empty($_POST['art_tytul']) AND !empty($_POST['art_tresc'])) {
     ...
     $sql = mysqli_query($this->$link,
     "INSERT INTO `artykul`
     (`art_id`, `uzy_id`, `art_tytul`, `art_tresc`, `art_data`, `art_obraz`) VALUES
     ...
     foreach ($row as $key => $val) {
       ...
       <ul>
         ...
         <li>
           <p>'.$val['art_tytul'].'</p>
           <span>
             <a href="?page=articles&action=edit&id='.$val['art_id'].'"><i class="fas fa-pen-square"></i>Edytuj</a>
             <a href="?page=articles&action=delete&id='.$val['art_id'].'"><i class="fas fa-trash"></i>Usuń</a>
           </span>
         </li>
         <li>'.$val['art_data'].'</li>
       </ul>
 ...
 // wyswietlenie wszystkich artykulow
  function showArticleFront(){
    ...
    if (empty($_GET['artykul']) && empty($_GET['kategoria'])) {
      ...
      $data = date('Y-m-d');
      $sql = mysqli_query($this->$link,
      "INSERT INTO `statystyka` (`stat_id`, `stat_data`, `art_id`)
      VALUES (NULL, '$data', $idart);");
